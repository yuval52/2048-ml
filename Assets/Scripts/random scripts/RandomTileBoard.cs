...
public class RandomTileBoard : MonoBehaviour
{
    [SerializeField] private RandomTile tilePrefab;
    [SerializeField] private TileState[] tileStates;

    public RandomTileGrid grid;
    //public RandomTileGrid demoGrid;
    ...
    private void Awake()
    {
        //grid = GetComponentInChildren<RandomTileGrid>();
        //manager = GetComponent<RandomGameManager>();
        tiles = new List<RandomTile>(16);
    ...
    {
        RandomTile tile = Instantiate(tilePrefab, grid.transform);
    ...
    private void Update(){
        ...
        if(lastMove >= (manager.moveTime * 2)){
    ...
    // public void WaitMakeMove(int move){
    //     currentMove = move;
    //     Invoke("MakeMove()", 0.1f);
    // }
    ...
    private bool Move(Vector2Int direction, int startX, int incrementX, int startY, int incrementY)
    {
        ...
        for (int x = startX; x >= 0 && x < grid.Width; x += incrementX)
        {
            ...
            {
                RandomTileCell cell = grid.GetCell(x, y);
    ...
    private bool MoveTile(RandomTile tile, Vector2Int direction)
    {
        RandomTileCell newCell = null;
        RandomTileCell adjacent = grid.GetAdjacentCell(tile.cell, direction);
    ...
    private bool CanMerge(RandomTile a, RandomTile b)
    {
        return a.state == b.state && !b.locked;
    }

    private void MergeTiles(RandomTile a, RandomTile b)
    {
        tiles.Remove(a);
        a.Merge(b.cell);
        ...
        RandomGameManager.Instance.IncreaseScore(newState.number);
    }
    ...
    private IEnumerator WaitForChanges()
    {
        ...
        if (CheckForGameOver()) {
            RandomGameManager.Instance.GameOver();
        }
    ...
    public bool CheckForGameOver()
    {
        ...
        foreach (var tile in tiles)
        {
            RandomTileCell up = grid.GetAdjacentCell(tile.cell, Vector2Int.up);
            RandomTileCell down = grid.GetAdjacentCell(tile.cell, Vector2Int.down);
            RandomTileCell left = grid.GetAdjacentCell(tile.cell, Vector2Int.left);
            RandomTileCell right = grid.GetAdjacentCell(tile.cell, Vector2Int.right);
